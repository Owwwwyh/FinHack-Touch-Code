...
- token-001-valid.jws: Valid token, current timestamp
- token-002-expired.jws: Expired token (exp in past)
- token-003-bad-sig.jws: Valid structure but tampered signature
- token-004-missing-nonce.jws: Missing required nonce field
- token-005-tampered-amount.jws: Amount changed after signing
- token-006-unknown-kid.jws: Unknown device kid
"""
...
with open("/tmp/ml/test-vectors/token-001-valid.jws", "w") as f:
    f.write(token_001)
...
with open("/tmp/ml/test-vectors/token-002-expired.jws", "w") as f:
    f.write(token_002)
...
with open("/tmp/ml/test-vectors/token-003-bad-sig.jws", "w") as f:
...
with open("/tmp/ml/test-vectors/token-004-missing-nonce.jws", "w") as f:
    f.write(token_004)
...
with open("/tmp/ml/test-vectors/token-005-tampered-amount.jws", "w") as f:
...
with open("/tmp/ml/test-vectors/token-006-unknown-kid.jws", "w") as f:
    f.write(token_006)
print(f"✓ token-006-unknown-kid.jws")

print("\n✅ All 6 test vectors generated in /tmp/ml/test-vectors/")
